...
/**
    本函数与btts大致相同，不同处在于需传递一个已有的audio组件，而
    不是在JS代码中利用dom新建一个audio然后append到dom的最后。
    调用远程API获得语音后会直接将语音的url设置到传入的audio组件
    的src属性中。
    本函数目前仅用于myComment.jsp中
    @param: audio: 已有的要进行语音播放的HTML组件
 */
function myCommentBtts(param, options, audio){
    var url = 'http://tsn.baidu.com/text2audio';
    var opt = options || {};
    var p = param || {};
